...
export const validateCommand = new Command('validate')
  ...
  .option('-s, --skills', 'Validate as skills (default: auto-detect)')
  .option('-r, --rules', 'Validate as skills (alias for --skills, default: auto-detect)')
  ...
  .action(async (targetPath, options) => {
    ...
    if (!targetPath) {
      ...
      if (!options.rules && !options.agents && !options.skills) {
        console.log(chalk.yellow('\n⚠ No configuration found. Use --skills or --agents to specify a path.'));
        process.exit(1);
      }
    } else {
